...
#define CS_LOW()	PORTB &= ~(1<<PORTB2)	/* Set CS low */
#define	CS_HIGH()	PORTB |=  (1<<PORTB2)	/* Set CS high */
#define	IS_CS_LOW	!(PINB & (1<<PINB2))	/* Test if CS is low */
#define	FORWARD(d)	xmit(d)				/* Data streaming function (console out) */

void xmit (unsigned char data) {			/* suart.S: Send a byte via software UART */
	//unsigned bit might be a problem
...
void init_spi (void) {		/* usi.S: Initialize MMC control ports */
	DDRB |= (1<<PORTB3)|(1<<PORTB5);	//Set up output pins; MOSI and serial clock
	DDRB |= (1<<PORTB2); //CS
	DDRB &= ~(1<<PORTB4); //input pin = miso
	PORTB |= (1<<PORTB2); 
...
void xmit_spi (BYTE d) {		/* usi.S: Send a byte to the MMC */
	//PORTB &= ~(1<< PORTB2);	//CS pulled low to start communication

	SPDR = d;		//Place data to be sent on registers
	while(!(SPSR & (1<<SPIF)));	//wait for end of transmission

	//PORTB |= (1<< PORTB2);	//set CS to high
}
